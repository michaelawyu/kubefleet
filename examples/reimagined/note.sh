...
#####
export RAW_ORAS_REF=kubefleetocidemo.azurecr.io/testdata/raw:latest
oras push --artifact-type "${ORAS_ARTIFACT_TYPE}"  "${RAW_ORAS_REF}" app/ ns.yml README.md  

export KUSTOMIZE_ORAS_REF=kubefleetocidemo.azurecr.io/testdata/kustomize:latest
oras push --artifact-type "${ORAS_ARTIFACT_TYPE}" "${KUSTOMIZE_ORAS_REF}" base/ overlays/ README.md  

kubectl create secret docker-registry acr-access -n work \
  --docker-server=kubefleetocidemo.azurecr.io \
  --docker-username=kubefleetocidemo \
  --docker-password=7ZUb80ODAPSLwsWPUoIXbEdA1A2e2g74kZTNZ4FQfn4h91F7sOSKJQQJ99CGAC3pKaREqg7NAAACAZCRR2tC \
  --docker-email=chenyu1@microsoft.com

kubectl apply -f examples/reimagined/oras.raw.yaml

kubectl annotate orasmanifests raw -n work experimental.kubefleet.dev/place-to-regions=centralus --overwrite
...
kubectl apply -f examples/reimagined/oras.kustomize.1.yaml
kubectl annotate orasmanifests k1 -n work experimental.kubefleet.dev/place-to-regions=eastus --overwrite

kubectl apply -f examples/reimagined/oras.kustomize.2.yaml
kubectl annotate orasmanifests k2 -n work experimental.kubefleet.dev/place-to-regions=westus2 --overwrite