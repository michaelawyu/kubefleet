...
func (g *ServiceAccountsAndTokenRequestsValidatingAdmissionPolicyGenerator) Validate() error {
	...
	for _, username := range g.WhitelistedUsernames {
		if strings.ContainsAny(username, `"\`) {
			return errors.NewUserError(nil, "whitelisted username contains illegal characters for a CEL expression", "username", username)
		}
	}
	for _, userGroup := range g.WhitelistedUserGroups {
		if strings.ContainsAny(userGroup, `"\`) {
			return errors.NewUserError(nil, "whitelisted user group contains illegal characters for a CEL expression", "userGroup", userGroup)
		}
	}
	return nil
}
...
// PolicyBindings generates a ValidatingAdmissionPolicyBinding for the ValidatingAdmissionPolicy
// generated by the Policies() method.
func (g *ServiceAccountsAndTokenRequestsValidatingAdmissionPolicyGenerator) PolicyBindings() []*admissionregistrationv1.ValidatingAdmissionPolicyBinding {
